...
class Test {
    ...
    fun Team.memberInJob(job: Job): Boolean {
    ...
    fun Team.memberAvgYear(): Int {
        val members = toMemeber()
        return members.sumBy { it.years } / members.size
    }
    ...
    fun run() {
        ...
        val firstTeams2: List<Team> = teams
                .asSequence()
                .filter { it.isMemeberSizeEqualOrOver(3) }
                .filter { it.memberInJob(Job.Designer) }
                .filter { it.memberAvgYear()>=10 }
        ...
        val secondTeams2: List<Team> = teams
                .asSequence()
                .filter { it.isMemeberSizeEqualOrOver(3) }
        ...
        val finalTeam: Team? = teams
                ...
                .filter { it -> !it.toMemeber().none { it.years >=5 } }
                .filter { it.properties == IncentiveSettings.Even }
                .maxBy { it -> it.toMemeber().sumBy { it.years } }

        //출력 finalTeam


        println("firstTeams--------------")
        firstTeams.take(3) .forEach {
            println(it)
        }
        println("firstTeams2--------------")
        firstTeams2.take(3) .forEach {
            println(it)
        }
        ...
        println("secondTeams2--------------")
        secondTeams2.take(3).forEach {
            println(it)
        }
