...
object SalaryDelegate {
    operator fun getValue(person: Person, property: KProperty<*>): Int {
        return EmployeeRepository.employees.single { it.id == person.id }.salary
    }

    operator fun setValue(person: Person, property: KProperty<*>, salary: Int) {
        val employee = EmployeeRepository.employees.single { it.id == person.id }
        val oldSalary = employee.salary
        employee.salary = salary
        println("${person.name} 님의 급여가 $oldSalary 에서 ${salary}로 변경되었습니다.")
    }
}
