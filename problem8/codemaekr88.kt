...
//무공변성 : 상속 관계 없이 자기 타입만 허용
//공변성 : 자기 자신과 자식 객체를 허용. out 키워드 사용. 리스코프 치환 법칙 허용.
//반공변성 : 자기 자신과 부모 객체만 허용. in 키워드 사용.
...
- 2번은 컴파일이 (실패) 한다. 왜냐하면 [파라미터로 넘기는 String은 Number?의 하위 타입이 아니다.]
- 3번은 컴파일이 (성공) 한다. 왜냐하면 [파라미터로 넘기는 Int? Number?의 하위 타입 이다.]
- 4번은 컴파일이 (실패) 한다. 왜냐하면 [파라미터로 넘기는 String?은 Number?의 하위 타입이 아니다.]
- 5번은 컴파일이 (성공) 한다. 왜냐하면 [파라미터로 넘기는 null은 Number?의 하위 타입 이다.]
...
fun main(args: Array<String>) {
    ...
    // 따라서 기저 타입인 ItemConsumer, ItemProducer 가 같아도, 타입 인자가 다르면 [하위 타입 관계가 성립하지 않는다.- 타입 불일치]
    // (무공변성)을 만족하기 위해서는,
