...
- 2번은 컴파일이 실패 한다. 왜냐하면 파라미터로 넘기는 String은 Number? 와 상/하위 타입관계가 성립하지 않기 때문이다.
- 3번은 컴파일이 성공 한다. 왜냐하면 파라미터로 넘기는 Int?는 Number의 하위 타입이기 때문이다.
- 4번은 컴파일이 실패 한다. 왜냐하면 파라미터로 넘기는 String?은 Number? 와 상/하위 타입관계가 성립하지 않기 때문이다.
- 5번은 컴파일이 성공 한다. 왜냐하면 파라미터가 nullable 이기 때문이다.
...
fun main(args: Array<String>) {
    ...
    // 따라서 기저 타입인 ItemConsumer, ItemProducer 가 같아도, 타입 인자가 다르면 상/하위 타입 관계가 아니다.
    // 공변성을 만족하기 위해서는,
