...
타입 A의 값이 필요한 모든 장소에 어떤 타입 B의 값을 넣어도 아무 문제가 없다면 타입 B는 타입 A의 [하위타입]이다.

- Int는 Number의 [하위타입] 이다.
- Int는 Number?의 [하위타입] 이다.
- String?은 String의 [상위타입] 이다.
- Any? 는 모든 타입의 [상위타입] 이다.
- List<String>은 Any의 [하위타입] 이다.
...
- 2번은 컴파일이 (실패) 한다. 왜냐하면 [ String이 Number?의 하위타입이 아니므로 ]
- 3번은 컴파일이 (성공) 한다. 왜냐하면 [ Int?는 Number?의 하위타입이므로 ]
- 4번은 컴파일이 (실패) 한다. 왜냐하면 [ String?은 Number?의 하위타입이 아니므로 ]
- 5번은 컴파일이 (성공) 한다. 왜냐하면 [ nullable이므로 ]
...
fun main(args: Array<String>) {
    ...
    // 따라서 기저 타입인 ItemConsumer, ItemProducer 가 같아도, 타입 인자가 다르면 [ 상/하위 관계가 아니다 ]
    // (공변성)을 만족하기 위해서는,
    ...
    // fun <T> produceAndConsume(consumer: ItemConsumer<T>, producer: ItemProducer<out T>) {


    produceAndConsume<Int>(numberConsumer, intProducer)
    // Number는 Int의 상위 타입이기 때문에,
    // Number를 Consume 가능하다면, Int 또한 Consume이 가능하다는 것이 보장된다.
    // 따라서 편의를 위해 ItemConsumer<Number> 가 ItemConsumer<Int> 의 (하위)타입이 되도록 만들고 싶다.
    // 이 관계는 (반공변성)이고, 이를 표현하기 위해서 [in] 키워드를 사용하면 된다.
    // 위에 서술한대로 produceAndConsume 함수를 수정해 주세요.
    // fun <T> produceAndConsume(consumer: ItemConsumer<in T>, producer: ItemProducer<T>) {
