...
- 2번은 컴파일이 (실패) 한다. 왜냐하면 [ String은 Number?의 하위 타입이 아니기 때문 이다. ]
- 3번은 컴파일이 (성공) 한다. 왜냐하면 [ Int?=null은 Number?의 하위 타입이기 때문이다. ]
- 4번은 컴파일이 (실패) 한다. 왜냐하면 [ String?은 Number?의 하위 타입이 아니기 때문 이다. ]
- 5번은 컴파일이 (성공) 한다. 왜냐하면 [ null은 Number?의 상한 타입이기 때문 이다. ]
*/
...
fun <T> createList(vararg items: T) : List<*>{
    return items.toList()
}
...
fun main(args: Array<String>) {
    ...
    // 따라서 기저 타입인 ItemConsumer, ItemProducer 가 같아도, 타입 인자가 다르면 [ 타입불일치 이다. ]
    // (무공변성)을 만족하기 위해서는,
...
fun <T> produceAndConsumeWithPrint(consumer: ItemConsumer<in T>, producer: ItemProducer<out T>) {
