...
fun main(args: Array<String>) {
    ...
    // 함수 호출이 실패하는 이유 :
    // produceAndConsume 의 T는 아무런 키워드도 없으므로 무공변성이다.
    // 따라서 기저 타입인 ItemConsumer, ItemProducer 가 같아도, 타입 인자가 다르면 상,하위 타입이 성립하지 않는다.
    // 무공변성을 만족하기 위해서는,
    // ItemConsumer 와 ItemProducer 의 타입인자가 각각 [T], [T] 이어야 한다.
    ...
    // 따라서 편의를 위해 ItemProducer<Int> 가 ItemProducer<Number> 의 하위타입이 되도록 만들고 싶다.
    // 이 관계는 공변성이고, 이를 표현하기 위해서 [out] 키워드를 사용하면 된다.
    ...
    // 따라서 편의를 위해 ItemConsumer<Number> 가 ItemConsumer<Int> 의 하위타입 되도록 만들고 싶다.
    // 이 관계는 반공변성이고, 이를 표현하기 위해서 [in] 키워드를 사용하면 된다.
    // 위에 서술한대로 produceAndConsume 함수를 수정해 주세요.
    ...
    // 따라서 함수마다 반복하지 않도록 선언 지점 변성을 사용하면 코드를 간결하게 할 수 있다.
