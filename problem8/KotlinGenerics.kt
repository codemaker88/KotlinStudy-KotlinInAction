...
타입 A의 값이 필요한 모든 장소에 어떤 타입 B의 값을 넣어도 아무 문제가 없다면 타입 B는 타입 A의 [ ]이다.

- Int는 Number의 [ ] 이다.
- Int는 Number?의 [ ] 이다.
- String?은 String의 [ ] 이다.
- Any? 는 모든 타입의 [ ] 이다.
- List<String>은 Any의 [ ] 이다.
...
- 2번은 컴파일이 (성공/실패) 한다. 왜냐하면 [ ]
- 3번은 컴파일이 (성공/실패) 한다. 왜냐하면 [ ]
- 4번은 컴파일이 (성공/실패) 한다. 왜냐하면 [ ]
- 5번은 컴파일이 (성공/실패) 한다. 왜냐하면 [ ]
...
class ItemConsumer<T> {
    fun consume(item: T) {
        println(item.toString())
    }
}

class ItemProducer<T> (private val item: T) {
    fun produce(): T {
        return item
    }
}

fun <T> produceAndConsume(consumer: ItemConsumer<T>, producer: ItemProducer<T>) {
...
fun main(args: Array<String>) {
    ...
    // 함수 호출이 실패하는 이유 :
    // produceAndConsume 의 T는 아무런 키워드도 없으므로 (무공변성 / 공변성 / 반공변성)이다.
    // 따라서 기저 타입인 ItemConsumer, ItemProducer 가 같아도, 타입 인자가 다르면 [ ]
    // (무공변성 / 공변성 / 반공변성)을 만족하기 위해서는,
    // ItemConsumer 와 ItemProducer 의 타입인자가 각각 [ ], [ ] 이어야 한다.
    ...
    // 따라서 편의를 위해 ItemProducer<Int> 가 ItemProducer<Number> 의 (상위 / 하위)타입이 되도록 만들고 싶다.
    // 이 관계는 (무공변성 / 공변성 / 반공변성)이고, 이를 표현하기 위해서 [ ] 키워드를 사용하면 된다.
    ...
    // 따라서 편의를 위해 ItemConsumer<Number> 가 ItemConsumer<Int> 의 (상위 / 하위)타입이 되도록 만들고 싶다.
    // 이 관계는 (무공변성 / 공변성 / 반공변성)이고, 이를 표현하기 위해서 [ ] 키워드를 사용하면 된다.
    // 위에 서술한대로 produceAndConsume 함수를 수정해 주세요.
    ...
    // Producer는 (in / out) 위치의 함수만 가지고 있고,
    // Consumer는 (in / out) 위치의 함수만 가지고 있기 때문이다.
    // 따라서 함수마다 반복하지 않도록 (선언 지점 변성 / 사용 지점 변성)을 사용하면 코드를 간결하게 할 수 있다.
